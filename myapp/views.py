...
from .models import User, Bus, Book, UserFeedBack
...
def signup(request):
    ...
    if request.method == 'POST':
        ...
        if not email_r.endswith('@gmail.com'):
            context["error"] = "Please provide a Gmail address."
            return render(request, 'myapp/signup.html', context)

        # Check if username contains only alphabetic characters and underscores
        if not re.match(r'^[a-zA-Z_]+$', name_r):
            context["error"] = "Please provide the username with only alphabetic characters and underscores."
            return render(request, 'myapp/signup.html', context)

        # Check if password contains at least one special character and is at least 8 characters long
        if not re.match(r'^.*[!@#$%^&*()_+}{:;?.><,|].*$', password_r) or len(password_r) < 8:
            context["error"] = "Please provide a password with at least one special character and at least 8 characters long."
            return render(request, 'myapp/signup.html', context)
